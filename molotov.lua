...
local function throw_cocktail(item, player)
	...
	local obj = minetest.add_entity({x=playerpos.x,y=playerpos.y+1.625,z=playerpos.z}, 'rangedweapons:molotov_entity')
	...
	if self.timer>0.2 then
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= 'rangedweapons:molotov_entity' and obj:get_luaentity().name ~= '__builtin:item' then
					if self.node ~= '' then
...
minetest.register_entity('rangedweapons:molotov_entity', {
	timer=0,
...
minetest.register_craftitem('rangedweapons:molotov_cocktail', {
...
minetest.register_craft({
	output = "rangedweapons:molotov_cocktail 7",
	recipe = {
		{"vessels:glass_bottle", "farming:cotton", "vessels:glass_bottle"},
		{"vessels:glass_bottle", "farming:ethanol", "vessels:glass_bottle"},
...
minetest.register_craft({
	type = 'fuel',
	recipe = 'rangedweapons:molotov_cocktail',
	burntime = 5,
})
