...
local function update_fence(pos)
    if minetest.get_node(pos).name:find("rangedweapons:chainlink") == nil then
        return
    end
    local sum = 0
    for i = 1, 4 do
    ...
    minetest.add_node(pos, {name = "rangedweapons:chainlink_"..sum})
end
...
for i = 1, 15 do
    ...
    local texture = "rangedweapons_chainlink.png"
    if cnt == 1 then
        texture = "rangedweapons_chainlink.png"
    end
    minetest.register_node("rangedweapons:chainlink_"..i, {
        drawtype = "nodebox",
        tile_images = {"rangedweapons_chainlink.png", "rangedweapons_chainlink.png", texture},
        paramtype = "light",
        groups = {cracky=2},
        drop = "rangedweapons:chainlink",
        node_box = {
            type = "fixed",
            fixed = take
        },
...
minetest.register_node("rangedweapons:chainlink", {
    description = "Chainlink Fence",
    tiles = {"rangedweapons_chainlink.png"},
    inventory_image = "rangedweapons_chainlink.png",
    wield_image = "rangedweapons_chainlink.png",
...
minetest.register_craft({
	output = "rangedweapons:chainlink 12",
	recipe = {
